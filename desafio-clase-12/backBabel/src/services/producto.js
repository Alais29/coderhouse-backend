...
export class Productos {
  ...
  async getProducto(id) {
    try {
      ...
      const producto = productosJSON.filter((item) => item.id === id)[0];
      return producto;
    } catch (e) {
  ...
  async updateProducto(id, producto) {
    try {
      ...
      let productToUpdate = productosJSON.find((item) => item.id === id);
      productToUpdate = {
        ...productToUpdate,
        ...producto,
      };

      const newProductList = productosJSON.filter((item) => item.id !== id);
